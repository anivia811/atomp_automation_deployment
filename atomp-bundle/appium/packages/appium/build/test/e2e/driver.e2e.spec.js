...
const caps = helpers_1.W3C_PREFIXED_CAPS;
const wdOpts = {
    hostname: helpers_1.TEST_HOST,
    connectionRetryCount: 0,
};
async function initFakeDriver(appiumHome) {
    const { driverConfig } = await (0, extension_2.loadExtensions)(appiumHome);
...
describe('FakeDriver via HTTP', function () {
    ...
    function withServer(args = {}) {
        ...
        before(async function () {
            const merged = { ...args, appiumHome, port, address: helpers_1.TEST_HOST };
            if (shouldStartServer) {
                server = await (0, main_1.main)(merged);
            }
        });
        // eslint-disable-next-line mocha/no-sibling-hooks
        after(async function () {
    ...
    describe('server updating', function () {
        ...
        it('should allow drivers to update the server in arbitrary ways', async function () {
            const { data } = await axios_1.default.get(`${testServerBaseUrl}/fakedriver`);
        ...
        it('should update the server with cliArgs', async function () {
            ...
            // arg got through.
            expect((await axios_1.default.post(`http://${helpers_1.TEST_HOST}:${port}/fakedriverCliArgs`)).data).to.have.property('appiumHome', appiumHome);
        });
    });
    describe('cli args handling for empty args', function () {
        withServer();
        it('should not receive user cli args if none passed in', async function () {
            const driver = await (0, webdriverio_1.remote)({ ...wdOpts, capabilities: caps });
            const { sessionId } = driver;
            try {
                const { data } = await axios_1.default.get(`${testServerBaseSessionUrl}/${sessionId}/fakedriverargs`);
    ...
    describe('default capabilities via cli', function () {
        ...
        it('should allow appium-prefixed caps sent via appium:options through --default-capabilities', async function () {
            ...
            // Create the session
            const { value } = (await axios_1.default.post(testServerBaseSessionUrl, appiumOptsCaps)).data;
            try {
                ...
                // End session
                await axios_1.default.delete(`${testServerBaseSessionUrl}/${value.sessionId}`);
            }
        });
    });
    describe('inspector commands', function () {
        ...
        it('should list available driver commands', async function () {
            driver.addCommand('listCommands', async () => (await axios_1.default.get(`${testServerBaseSessionUrl}/${driver.sessionId}/appium/commands`)).data
                .value);
            const commands = await driver.listCommands();
            expect(JSON.stringify(commands.rest.base['/session/:sessionId/frame'])).to.eql(JSON.stringify({ POST: { command: 'setFrame', params: [{ name: 'id', required: true }] } }));
            expect(Object.keys(commands.rest.driver).length).to.be.greaterThan(1);
            expect(JSON.stringify(commands.bidi.base.session.subscribe)).to.eql(JSON.stringify({
        ...
        it('should list available driver extensions', async function () {
            driver.addCommand('listExtensions', async () => (await axios_1.default.get(`${testServerBaseSessionUrl}/${driver.sessionId}/appium/extensions`))
                .data.value);
            const extensions = await driver.listExtensions();
            expect(JSON.stringify(extensions.rest.driver['fake: setThing'])).to.eql(JSON.stringify({
    ...
    describe('session handling', function () {
        ...
        it('should be able to run two FakeDriver sessions simultaneously', async function () {
            const driver1 = await (0, webdriverio_1.remote)({ ...wdOpts, capabilities: caps });
            expect(driver1.sessionId).to.exist;
            expect(driver1.sessionId).to.be.a('string');
            const driver2 = await (0, webdriverio_1.remote)({ ...wdOpts, capabilities: caps });
        ...
        it('should not be able to run two FakeDriver sessions simultaneously when one is unique', async function () {
            ...
            const driver1 = await (0, webdriverio_1.remote)({ ...wdOpts, capabilities: uniqueCaps });
            expect(driver1.sessionId).to.exist;
            expect(driver1.sessionId).to.be.a('string');
            await expect((0, webdriverio_1.remote)({ ...wdOpts, capabilities: caps })).to.eventually.be.rejected;
            await driver1.deleteSession();
        });
        it('should use the newCommandTimeout of the inner Driver on session creation', async function () {
            ...
            const driver = await (0, webdriverio_1.remote)({ ...wdOpts, capabilities: localCaps });
            expect(driver.sessionId).to.exist;
        ...
        it('should not allow umbrella commands to prevent newCommandTimeout on inner driver', async function () {
            ...
            const driver = await (0, webdriverio_1.remote)({ ...wdOpts, capabilities: localCaps });
            expect(driver.sessionId).to.exist;
            driver.addCommand('getStatus', async () => (await axios_1.default.get(`${testServerBaseUrl}/status`)).data.value);
        ...
        it('should accept valid W3C capabilities and start a W3C session', async function () {
            ...
            const w3cCaps = {
                capabilities: {
                    ...
                    firstMatch: [{ 'appium:deviceName': 'Fake', 'appium:app': helpers_1.TEST_FAKE_APP }],
                },
            };
            // Create the session
            const { status, value, sessionId } = (await axios_1.default.post(testServerBaseSessionUrl, w3cCaps)).data;
            try {
                ...
                const { status: screenshotStatus, value: screenshotValue } = (await (0, axios_1.default)({
                ...
                await expect(axios_1.default.post(`${testServerBaseSessionUrl}/${value.sessionId}/execute/async`, {
                    script: '',
                    args: ['a'],
        ...
        it('should allow appium-prefixed caps sent via appium:options', async function () {
            ...
            // Create the session
            const { status, value, sessionId } = (await axios_1.default.post(testServerBaseSessionUrl, appiumOptsCaps)).data;
            try {
                expect(status).to.not.exist; // Test that it's a W3C session
                expect(sessionId).to.not.exist;
                expect(value.sessionId).to.be.a('string');
        ...
        it('should reject invalid W3C capabilities and respond with a 400 Bad Parameters error', async function () {
            ...
            await expect(axios_1.default.post(testServerBaseSessionUrl, badW3Ccaps)).to.eventually.be.rejectedWith(/400/);
        });
        it('should accept a combo of W3C and JSONWP capabilities but completely ignore JSONWP', async function () {
            ...
            const { status, value, sessionId } = (await axios_1.default.post(testServerBaseSessionUrl, combinedCaps))
                .data;
            try {
                ...
                expect(value.capabilities).to.deep.equal({
                    ...(0, capability_1.removeAppiumPrefixes)(caps),
                    w3cParam: 'w3cParam',
                });
            }
            finally {
        ...
        it('should reject bad automation name with an appropriate error', async function () {
            ...
            await expect(axios_1.default.post(testServerBaseSessionUrl, w3cCaps)).to.eventually.be.rejectedWith(/500/);
        });
        it('should accept capabilities that are provided in the firstMatch array', async function () {
            ...
            const { value, sessionId, status } = (await axios_1.default.post(testServerBaseSessionUrl, w3cCaps)).data;
            try {
                expect(status);
                expect(sessionId);
                expect(value.capabilities).to.deep.equal((0, capability_1.removeAppiumPrefixes)(caps));
            }
            finally {
                // End session
                await axios_1.default.delete(`${testServerBaseSessionUrl}/${value.sessionId}`);
        ...
        it('should not fall back to MJSONWP if w3c caps are invalid', async function () {
            ...
            const res = await axios_1.default.post(testServerBaseSessionUrl, combinedCaps, {
                validateStatus: null,
            });
        ...
        it('should not fall back to MJSONWP even if Inner Driver is not ready for W3C', async function () {
            ...
            const createSessionStub = sandbox
                ...
                const res = await base_driver_1.BaseDriver.prototype.createSession.call(this, caps);
        ...
        it('should allow drivers to update the method map with new routes and commands', async function () {
            ...
            const { sessionId } = driver;
            try {
                await axios_1.default.post(`${testServerBaseSessionUrl}/${sessionId}/fakedriver`, {
                    thing: { yes: 'lolno' },
                });
                expect((await axios_1.default.get(`${testServerBaseSessionUrl}/${sessionId}/fakedriver`)).data.value).to.eql({ yes: 'lolno' });
            }
            finally {
                await driver.deleteSession();
            }
    ...
    describe('Bidi protocol', function () {
        ...
        let driver;
        beforeEach(async function () {
            driver = (await (0, webdriverio_1.remote)({ ...wdOpts, capabilities }));
        });
        afterEach(async function () {
            if (driver) {
                await driver.deleteSession();
    ...
    describe('Bidi protocol with base path', function () {
        ...
        let driver;
        beforeEach(async function () {
            driver = (await (0, webdriverio_1.remote)({ ...wdOpts, path: basePath, capabilities }));
        });
        afterEach(async function () {
            if (driver) {
                await driver.deleteSession();
...
describe('Bidi over SSL', function () {
    async function generateCertificate(certPath, keyPath) {
        await (0, teen_process_1.exec)('openssl', [
            'req',
            '-nodes',
            '-new',
            '-x509',
    ...
    before(async function () {
        ...
        appiumHome = await support_1.tempDir.openDir();
        wdOpts.port = port = await (0, helpers_1.getTestPort)();
        testServerBaseUrl = `https://${helpers_1.TEST_HOST}:${port}`;
        FakeDriver = await initFakeDriver(appiumHome);
        server = await (0, main_1.main)({
    ...
    beforeEach(async function () {
        ...
        driver = await (0, webdriverio_1.remote)({ ...wdOpts, protocol: 'https', strictSSL: false, capabilities });
    });
    afterEach(async function () {
...
describe.skip('Logsink', function () {
    let server = null;
    const logs = [];
    const logHandler = function (level, message) {

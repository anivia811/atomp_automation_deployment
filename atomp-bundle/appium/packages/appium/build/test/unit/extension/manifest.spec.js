...
describe('Manifest', function () {
    ...
    beforeEach(function () {
        let overrides;
        ({ MockPackageChanged, MockAppiumSupport, MockGlob, overrides, sandbox } = (0, mocks_1.initMocks)());
        MockAppiumSupport.fs.readFile.resolves(yamlFixture);
        ({ Manifest } = helpers_1.rewiremock.proxy(() => require('../../../lib/extension/manifest'), {
            ...overrides,
    ...
    describe('instance method', function () {
        ...
        describe('read()', function () {
            ...
            describe('when the file is invalid YAML', function () {
                ...
                it('should reject', async function () {
                    await expect(manifest.read()).to.be.rejectedWith(Error, /trouble loading the extension installation cache file/i);
                });
            });
        ...
        describe('write()', function () {
            ...
            describe('when called after `read()`', function () {
                ...
                describe('when the manifest directory could not be created', function () {
                    ...
                    it('should reject', async function () {
                        await expect(manifest.write()).to.be.rejectedWith(Error, /could not create the directory for the manifest file/i);
                    });
                });
            });
        });
        describe('setExtension()', function () {
            ...
            it('should add a clone of the extension manifest to the internal data object', function () {
                manifest.setExtension(constants_1.DRIVER_TYPE, 'foo', extData);
                expect(manifest.getExtensionData(constants_1.DRIVER_TYPE).foo)
                    .to.eql(extData)
            ...
            describe('when existing extension added', function () {
                ...
                it('should rewrite', function () {
                    ...
                    manifest.setExtension(constants_1.DRIVER_TYPE, 'foo', expected);
                    expect(manifest.getExtensionData(constants_1.DRIVER_TYPE).foo).to.eql(expected);
                });
            });
            describe('when the extension has no peer dependency on `appium`', function () {
                ...
                it('should work anyway', function () {
                    manifest.setExtension(constants_1.DRIVER_TYPE, 'foo', extData);
                    expect(manifest.getExtensionData(constants_1.DRIVER_TYPE).foo).to.not.have.property('appiumVersion');
                });
            });
        });
        describe('getExtensionData()', function () {
            ...
            it('should return all extension data for an extension type', function () {
                expect(manifest.getExtensionData(constants_1.DRIVER_TYPE)).to.eql({ foo: extData });
            });
        });
        describe('addExtensionFromPackage()', function () {
            describe('when provided a valid package.json for a driver and its path', function () {
                ...
                it('should add an extension to the internal data', function () {
                    ...
                    expect(manifest.getExtensionData(constants_1.DRIVER_TYPE)).to.deep.equal({
                        myDriver: {
                            automationName: 'derp',
                            mainClass: 'SomeClass',
                            pkgName: 'derp',
            ...
            describe('when provided a valid package.json for a plugin and its path', function () {
                ...
                it('should add an extension to the internal data', function () {
                    manifest.addExtensionFromPackage(packageJson, '/some/path/to/package.json');
                    expect(manifest.getExtensionData(constants_1.PLUGIN_TYPE)).to.deep.equal({
                        myPlugin: {
                            mainClass: 'SomeClass',
                            pkgName: 'derp',
                            version: '1.0.0',
            ...
            describe('when provided a non-extension', function () {
                it('should throw', function () {
                    expect(() => manifest.addExtensionFromPackage({ herp: 'derp' }, '/some/path/to/package.json')).to.throw(/neither a valid driver nor a valid plugin/);
                });
            });
            describe('when the extension has an appium peer dependency beginning with `file:..`', function () {
                ...
                beforeEach(function () {
                    packageJson = {
                        ...
                        peerDependencies: {
                            appium: build_1.APPIUM_VER,
                        },
                    };
                });
                it('should set the appiumVersion to the current Appium version', function () {
                    manifest.addExtensionFromPackage(packageJson, '/some/path/to/package.json');
                    expect(manifest.getExtensionData(constants_1.DRIVER_TYPE).myDriver.appiumVersion).to.equal(build_1.APPIUM_VER);
                });
            });
        });
        describe('syncWithInstalledExtensions()', function () {
            beforeEach(function () {
                MockAppiumSupport.fs.readFile.resolves(JSON.stringify({
                    name: 'foo',
                    version: '1.0.0',
                    readme: 'stuff!',
                    _id: 'totally unique',
            ...
            it('should add a found extension', async function () {
                ...
                expect(manifest.getExtensionData(constants_1.DRIVER_TYPE)).to.have.property('myDriver');
            });
        ...
        describe('hasDriver()', function () {
            describe('when the driver is registered', function () {
                beforeEach(function () {
                    manifest.setExtension(constants_1.DRIVER_TYPE, 'foo', {});
                });
                it('should return `true`', function () {
        ...
        describe('hasPlugin()', function () {
            describe('when the plugin is registered', function () {
                beforeEach(function () {
                    manifest.setExtension(constants_1.PLUGIN_TYPE, 'foo', {});
                });
                it('should return `true`', function () {

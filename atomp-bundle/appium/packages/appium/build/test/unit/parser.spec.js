...
const config_file_1 = require("../../lib/bootstrap/config-file");
const helpers_1 = require("../helpers");
const { expect } = chai_1.default;
chai_1.default.use(chai_as_promised_1.default);
// these paths should not make assumptions about the current working directory
const ALLOW_FIXTURE = (0, helpers_1.resolveFixture)('allow-feat.txt');
const DENY_FIXTURE = (0, helpers_1.resolveFixture)('deny-feat.txt');
const CAPS_FIXTURE = (0, helpers_1.resolveFixture)('caps.json');
const LOG_FILTERS_FIXTURE = (0, helpers_1.resolveFixture)('log-filters.json');
describe('parser', function () {
    let p;
    describe('Main Parser', function () {
        beforeEach(async function () {
            p = await (0, parser_1.getParser)(true);
        });
        it('should accept only server and driver subcommands', function () {
            p.parseArgs([]);
            p.parseArgs(['server']);
            p.parseArgs([constants_1.DRIVER_TYPE, 'list']);
    ...
    describe('Server Parser', function () {
        describe('Appium arguments', function () {
            ...
            it('should parse --address correctly', function () {
                ...
                expect(p.parseArgs(['--address', '2a02:8888:9a80:158:2418:a474:43c6:1b78']).address).to.eql('2a02:8888:9a80:158:2418:a474:43c6:1b78');
            });
            ...
            it('should normalize hyphenated server args to dest form (normalizeServerArgs)', function () {
                ...
                parser_1.ArgParser.normalizeServerArgs(obj);
        ...
        describe('extension arguments', function () {
            beforeEach(async function () {
                ...
                await schema.registerSchema(constants_1.DRIVER_TYPE, 'fake', require('@appium/fake-driver/build/lib/fake-driver-schema').default);
            ...
            it('should parse driver args correctly from a string', async function () {
                ...
                const { config } = await (0, config_file_1.readConfigFile)((0, helpers_1.resolveFixture)('config', 'appium-config-driver-fake.json'));
                ...
                expect(args.driver.fake).to.eql(config?.driver?.fake);
            });
            it('should not yet apply defaults', function () {
                const args = p.parseArgs([]);
                expect(args).to.not.have.property(constants_1.DRIVER_TYPE);
            });
            it('should nicely handle extensions w/ dashes in them', async function () {
                schema.resetSchema();
                await schema.registerSchema(constants_1.PLUGIN_TYPE, 'crypto-fiend', {
                    type: 'object',
    ...
    describe('Driver Parser', function () {
        ...
        it('should not allow random sub-subcommands', function () {
            expect(() => p.parseArgs([constants_1.DRIVER_TYPE, 'foo'])).to.throw();
        });
        describe('list', function () {
            it('should allow an empty argument list', function () {
                const args = p.parseArgs([constants_1.DRIVER_TYPE, 'list']);
                expect(args.subcommand).to.eql(constants_1.DRIVER_TYPE);
            ...
            it('should allow json format', function () {
                const args = p.parseArgs([constants_1.DRIVER_TYPE, 'list', '--json']);
                expect(args.json).to.eql(true);
            });
            it('should allow --installed', function () {
                const args = p.parseArgs([constants_1.DRIVER_TYPE, 'list', '--installed']);
                expect(args.showInstalled).to.eql(true);
            });
            it('should allow --updates', function () {
                const args = p.parseArgs([constants_1.DRIVER_TYPE, 'list', '--updates']);
                expect(args.showUpdates).to.eql(true);
            });
            it('should allow "ls" as an alias for "list"', function () {
                const args = p.parseArgs([constants_1.DRIVER_TYPE, 'ls']);
                expect(args.subcommand).to.eql(constants_1.DRIVER_TYPE);
                expect(args.driverCommand).to.eql('list');
                expect(args.showInstalled).to.eql(false);
                expect(args.showUpdates).to.eql(false);
        ...
        describe('install', function () {
            it('should not allow an empty argument list', function () {
                expect(() => p.parseArgs([constants_1.DRIVER_TYPE, 'install'])).to.throw();
            });
            it('should take a driver name to install', function () {
                const args = p.parseArgs([constants_1.DRIVER_TYPE, 'install', 'foobar']);
                expect(args.subcommand).to.eql(constants_1.DRIVER_TYPE);
            ...
            it('should allow json format', function () {
                const args = p.parseArgs([constants_1.DRIVER_TYPE, 'install', 'foobar', '--json']);
                expect(args.json).to.eql(true);
            });
            it('should allow --source', function () {
                for (const source of extension_config_1.INSTALL_TYPES) {
                    const args = p.parseArgs([constants_1.DRIVER_TYPE, 'install', 'foobar', '--source', source]);
                    expect(args.installType).to.eql(source);
                }
            });
            it('should not allow unknown --source', function () {
                expect(() => p.parseArgs([constants_1.DRIVER_TYPE, 'install', 'fobar', '--source', 'blah'])).to.throw();
            });
        });
        describe('uninstall', function () {
            it('should not allow an empty argument list', function () {
                expect(() => p.parseArgs([constants_1.DRIVER_TYPE, 'uninstall'])).to.throw();
            });
            it('should take a driver name to uninstall', function () {
                const args = p.parseArgs([constants_1.DRIVER_TYPE, 'uninstall', 'foobar']);
                expect(args.subcommand).to.eql(constants_1.DRIVER_TYPE);
            ...
            it('should allow json format', function () {
                const args = p.parseArgs([constants_1.DRIVER_TYPE, 'uninstall', 'foobar', '--json']);
                expect(args.json).to.eql(true);
            });
        });
        describe('update', function () {
            it('should not allow an empty argument list', function () {
                expect(() => p.parseArgs([constants_1.DRIVER_TYPE, 'update'])).to.throw();
            });
            it('should take a driver name to update', function () {
                const args = p.parseArgs([constants_1.DRIVER_TYPE, 'update', 'foobar']);
                expect(args.subcommand).to.eql(constants_1.DRIVER_TYPE);
            ...
            it('should allow json format', function () {
                const args = p.parseArgs([constants_1.DRIVER_TYPE, 'update', 'foobar', '--json']);
                expect(args.json).to.eql(true);
            });
        });
        describe('run', function () {
            it('should not allow an empty driver argument list', function () {
                expect(() => p.parseArgs([constants_1.DRIVER_TYPE, 'run'])).to.throw();
            });
            it('should allow no driver scriptName', function () {
                const args = p.parseArgs([constants_1.DRIVER_TYPE, 'run', 'foo']);
                expect(args.subcommand).to.eql(constants_1.DRIVER_TYPE);
            ...
            it('should take a driverName and scriptName to run', function () {
                const args = p.parseArgs([constants_1.DRIVER_TYPE, 'run', 'foo', 'bar']);
                expect(args.subcommand).to.eql(constants_1.DRIVER_TYPE);
                expect(args.driverCommand).to.eql('run');
                expect(args.driver).to.eql('foo');
            ...
            it('should allow json format for driver', function () {
                const args = p.parseArgs([constants_1.DRIVER_TYPE, 'run', 'foo', 'bar', '--json']);
                expect(args.json).to.eql(true);
            });
            it('should not allow an empty plugin argument list', function () {
                expect(() => p.parseArgs([constants_1.PLUGIN_TYPE, 'run'])).to.throw();
            });
            it('should allow no plugin scriptName', function () {
                const args = p.parseArgs([constants_1.PLUGIN_TYPE, 'run', 'foo']);
                expect(args.subcommand).to.eql(constants_1.PLUGIN_TYPE);
            ...
            it('should take a pluginName and scriptName to run', function () {
                const args = p.parseArgs([constants_1.PLUGIN_TYPE, 'run', 'foo', 'bar']);
                expect(args.subcommand).to.eql(constants_1.PLUGIN_TYPE);
                expect(args.pluginCommand).to.eql('run');
                expect(args.plugin).to.eql('foo');
                expect(args.scriptName).to.eql('bar');
            ...
            it('should allow json format for plugin', function () {
                const args = p.parseArgs([constants_1.PLUGIN_TYPE, 'run', 'foo', 'bar', '--json']);
                expect(args.json).to.eql(true);
            });
        });
    });
    describe('Setup Parser', function () {
        ...
        it('should not allow random sub-subcommands', function () {
            expect(() => p.parseArgs([constants_1.SETUP_SUBCOMMAND, 'foo'])).to.throw();
        });
        describe('all', function () {
            it('should allow an empty argument mobile', function () {
                const args = p.parseArgs([constants_1.SETUP_SUBCOMMAND, 'mobile']);
                expect(args.subcommand).to.eql(constants_1.SETUP_SUBCOMMAND);
    ...
    describe('getParser() and process.argv[1]', function () {
        ...
        it('should not fail if process.argv[1] is undefined', async function () {
            ...
            const args = await (0, parser_1.getParser)();

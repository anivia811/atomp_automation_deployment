...
exports.AVAILABLE_MATCHING_FUNCTIONS = exports.AVAILABLE_DETECTORS = exports.MATCH_NEIGHBOUR_THRESHOLD = exports.DEFAULT_MATCH_THRESHOLD = void 0;
...
let cv;
exports.DEFAULT_MATCH_THRESHOLD = 0.5;
exports.MATCH_NEIGHBOUR_THRESHOLD = 10;
exports.AVAILABLE_DETECTORS = {
    AKAZE: 'AKAZE',
...
exports.AVAILABLE_MATCHING_FUNCTIONS = {
...
async function getImagesMatches(img1Data, img2Data, options = {}) {
    ...
    try {
        ...
        if (!(detectorName in exports.AVAILABLE_DETECTORS)) {
            throw new Error(`'${detectorName}' detector is unknown. ` +
                `Only ${JSON.stringify(Object.keys(exports.AVAILABLE_DETECTORS))} detectors are supported.`);
        }
        if (!(matchFunc in exports.AVAILABLE_MATCHING_FUNCTIONS)) {
            throw new Error(`'${matchFunc}' matching function is unknown. ` +
                `Only ${JSON.stringify(Object.keys(exports.AVAILABLE_MATCHING_FUNCTIONS))} matching functions are supported.`);
        }
        const detector = pool.add(new cv[exports.AVAILABLE_DETECTORS[detectorName]]());
        ...
        const matcher = pool.add(new cv.DescriptorMatcher(exports.AVAILABLE_MATCHING_FUNCTIONS[matchFunc]));
...
async function getImageOccurrence(fullImgData, partialImgData, options = {}) {
    ...
    const { visualize = false, threshold = exports.DEFAULT_MATCH_THRESHOLD, multiple = false, matchNeighbourThreshold = exports.MATCH_NEIGHBOUR_THRESHOLD, method = DEFAULT_MATCHING_METHOD, } = options;
    const pool = new autorelease_pool_1.OpenCvAutoreleasePool();
    try {
        ...
        try {
            ...
            throw new Error(`Cannot find any occurrences of the partial image in the full image. ` +
                `Original error: ${e?.message || String(e)}`, { cause: e });
        }
        if (visualize) {
...
async function cvMatToPng(mat) {
    return await (0, sharp_1.default)(node_buffer_1.Buffer.from(mat.data), {
        raw: {
            width: mat.cols,
            height: mat.rows,
            channels: 4,
...
async function cvMatFromImage(img) {
    ...
    const { data, info } = await (0, sharp_1.default)(img).ensureAlpha().raw().toBuffer({ resolveWithObject: true });

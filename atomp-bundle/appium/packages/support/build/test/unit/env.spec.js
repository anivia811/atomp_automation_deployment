...
function missingPackageJsonError() {
...
describe('env', function () {
    ...
    beforeEach(function () {
        const result = (0, mocks_1.initMocks)();
        ...
        env = helpers_1.rewiremock.proxy(() => require('../../lib/env'), overrides);
    ...
    describe('resolveManifestPath()', function () {
        describe('when appium is not resolvable from cwd', function () {
            it('should return a path relative to the default APPIUM_HOME', async function () {
                (0, chai_1.expect)(await env.resolveManifestPath()).to.equal(node_path_1.default.join(env.DEFAULT_APPIUM_HOME, env.MANIFEST_RELATIVE_PATH));
            });
        });
        describe('when provided an explicit APPIUM_HOME', function () {
            describe('when a manifest file exists there', function () {
                it('it should return the existing path', async function () {
                    (0, chai_1.expect)(await env.resolveManifestPath('/somewhere/over/the/rainbow')).to.equal(node_path_1.default.join('/somewhere/over/the/rainbow', env.MANIFEST_RELATIVE_PATH));
                });
            });
        });
    });
    describe('resolveAppiumHome()', function () {
        describe('when param is not absolute', function () {
            it('should reject', async function () {
                await (0, chai_1.expect)(env.resolveAppiumHome('foo')).to.be.rejectedWith(TypeError, /absolute/i);
            });
        });
        describe('when APPIUM_HOME is set in env', function () {
            describe('when APPIUM_HOME is absolute', function () {
                beforeEach(function () {
                    process.env.APPIUM_HOME = node_path_1.default.resolve(node_path_1.default.sep, 'some', 'appium-home');
                });
                it('should resolve APPIUM_HOME from env', async function () {
                    await (0, chai_1.expect)(env.resolveAppiumHome()).to.eventually.equal(process.env.APPIUM_HOME);
                });
            });
            describe('when APPIUM_HOME is relative', function () {
                beforeEach(function () {
                    process.env.APPIUM_HOME = node_path_1.default.join('some', 'appium-home');
                });
                it('should resolve to an absolute path', async function () {
                    await (0, chai_1.expect)(env.resolveAppiumHome()).to.eventually.equal(node_path_1.default.join(process.cwd(), process.env.APPIUM_HOME));
                });
            });
        });
            ...
            describe('when Appium is not resolvable from cwd', function () {
                describe('when `appium` is not a dependency of the package in the cwd', function () {
                    beforeEach(function () {
                        ...
                        MockReadPackage.readPackage.resolves(undefined);
                    });
                    it('should resolve with DEFAULT_APPIUM_HOME', async function () {
                        await (0, chai_1.expect)(env.resolveAppiumHome('/somewhere')).to.eventually.equal(env.DEFAULT_APPIUM_HOME);
                    });
                });
                describe('when `appium` is a dependency of the package in the cwd', function () {
                    const appiumHome = node_path_1.default.resolve(node_path_1.default.sep, 'somewhere');
                    describe('when `appium` is a dependency which does not resolve to a file path', function () {
                        ...
                        it('should resolve with the identity', async function () {
                            await (0, chai_1.expect)(env.resolveAppiumHome(appiumHome)).to.eventually.equal(appiumHome);
                        });
                    });
                    describe('when `appium` is a dependency for version 0.x', function () {
                        beforeEach(function () {
                            MockReadPackage.readPackage.resolves({ devDependencies: { appium: '0.9.0' } });
                        });
                        it('should resolve with DEFAULT_APPIUM_HOME', async function () {
                            await (0, chai_1.expect)(env.resolveAppiumHome(appiumHome)).to.eventually.equal(env.DEFAULT_APPIUM_HOME);
                        });
                    });
                    describe('when `appium` is a dependency for version 1.x', function () {
                        beforeEach(function () {
                            MockReadPackage.readPackage.resolves({ devDependencies: { appium: '1.2.3' } });
                        });
                        it('should resolve with DEFAULT_APPIUM_HOME', async function () {
                            await (0, chai_1.expect)(env.resolveAppiumHome(appiumHome)).to.eventually.equal(env.DEFAULT_APPIUM_HOME);
                        });
    ...
    describe('readPackageInDir()', function () {
        it('should read package.json from the given directory', async function () {
            ...
            (0, chai_1.expect)(MockReadPackage.readPackage.calledWithExactly({
                cwd: '/somewhere',
                normalize: true,
            })).to.be.true;
        });
        it('should resolve with undefined when package.json is missing', async function () {
            MockReadPackage.readPackage.rejects(missingPackageJsonError());
            await (0, chai_1.expect)(env.readPackageInDir('/somewhere')).to.eventually.be.undefined;
        });
        it('should reject when reading package.json fails for reasons other than ENOENT', async function () {
            MockReadPackage.readPackage.rejects(new Error('on the fritz'));
            await (0, chai_1.expect)(env.readPackageInDir('/somewhere')).to.be.rejectedWith('on the fritz');
        });
    });
        ...
        describe('when Appium is not resolvable from cwd', function () {
            describe('when `appium` is not a dependency of the local package', function () {
                ...
                it('should resolve `false`', async function () {
                    await (0, chai_1.expect)(env.hasAppiumDependency('/somewhere')).to.eventually.equal(false);
                });
            ...
            describe('when `appium` is a dependency of the local package', function () {
                ...
                describe('when `appium` is not yet actually installed', function () {
                    ...
                    describe('when `appium` dep is current', function () {
                        beforeEach(function () {
                            MockReadPackage.readPackage.resolves({ devDependencies: { appium: '2.0.0' } });
                        });
                        it('should resolve `true`', async function () {
                            await (0, chai_1.expect)(env.hasAppiumDependency('/somewhere')).to.eventually.equal(true);
                        });
                    });
                    describe('when `appium` dep is v1.x', function () {
                        beforeEach(function () {
                            MockReadPackage.readPackage.resolves({ optionalDependencies: { appium: '1.x' } });
                        });
                        it('should resolve `false`', async function () {
                            await (0, chai_1.expect)(env.hasAppiumDependency('/somewhere')).to.eventually.equal(false);
                        });
                    ...
                    describe('when `appium` dep is v0.x', function () {
                        beforeEach(function () {
                            MockReadPackage.readPackage.resolves({ dependencies: { appium: '0.x' } });
                        });
                        it('should resolve `false`', async function () {
                            await (0, chai_1.expect)(env.hasAppiumDependency('/somewhere')).to.eventually.equal(false);
                        });

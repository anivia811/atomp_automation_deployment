...
# FastAPI/uvicorn app; the web apps reach it in-network as http://atomp-ai:8000
# (they run on atomp_automation_network, so "localhost" would be the web
# container itself, not this service). Uvicorn's default port :8000 is used to
# match *_AI_HOST below and origin/master's aeffa54b convention. start_container
# (not recreate) so the heavy conda/torch model load isn't repeated on every
# redeploy.
...
recreate_container tester40-web \
  ...
  -e TESTER40_AI_HOST="http://atomp-ai:8000" \

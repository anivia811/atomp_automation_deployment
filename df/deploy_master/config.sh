...
# -----------------------------------------
# WEB
# Kept as "dev" intentionally: the STF app unit uses --env only to decide
# whether the session cookie is marked Secure. This deployment has no TLS
# termination anywhere (plain HTTP), so a Secure cookie is never stored by
# the browser, which breaks login into Device Farm with an infinite
# redirect loop. If this deployment is ever put behind real HTTPS, switch
# this to "prod" so the session cookie gets marked Secure again.
export BUILD_ENV="dev"
export SECURED=""
export SECRET_KEY="atom@nuclear!"
export DEVICEFARM_IMAGE="devicefarm:b-20260710"
...
export ADMINISTRATOR="atom@gst.io,admin@hae.com,admin2@hae.com,admin@atomp.io"

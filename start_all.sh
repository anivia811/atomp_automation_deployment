...
###############################################
# 7. Start ATOMID
#    DB deps (atomid-mysql/redis) via compose, then atomid itself via
#    run_atomid.sh -- the atomid container is a plain `docker run` (new local
#    image + express5 shim + patched main.js + host port 8081), NOT compose.
###############################################
...
echo "[INFO] Starting ATOMID databases (atomid-mysql / atomid-redis)..."
(
  cd "$ATOMID_DIR"
  export DOCKER_NET_IP_ATOMID=10.20.0.2
  export DOCKER_NET_IP_MYSQL=10.20.0.3
  export DOCKER_NET_IP_REDIS=10.20.0.4
  docker compose up --detach atomid-mysql atomid-redis
)
echo "[INFO] Waiting 15s for atomid-mysql..."
sleep 15

# 7b. ATOMID app via run_atomid.sh (needs image present -> build_image/build_atomid.sh)
if docker image inspect "$ATOMID_IMAGE" &>/dev/null; then
  bash "$ATOMID_DIR/run_atomid.sh" || echo "[WARN] run_atomid.sh returned non-zero"
else
  echo "[WARN] $ATOMID_IMAGE missing -> build it: bash build_image/build_atomid.sh (skipping ATOMID)"
fi
